...
import {Platform, StyleSheet, Text, View, TextInput, Image, StatusBar, ScrollView, Alert} from 'react-native';
...
export default class HomeTopView extends Component<Props> {
    ...
    renderIndicator() {
        ...
        for (var i = 0; i < 2; i++) {
            //== 一般等于  ====严格等于
            // Alert.alert('i==this.state.currenentPage:' + (i == this.state.currenentPage)+',i:'+i+',this.state.currenentPage:'+this.state.currenentPage)
            ...
            <View style={styles.containStyle}>
                ...
                <ScrollView horizontal={true}
                            ...
                            onMomentumScrollEnd={(e) => {
                                // Alert.alert('x:' + e.nativeEvent.contentOffset.x + ',width:' + width)
                                // this.setState({currenentPage:(e.nativeEvent.contentOffset.x/width-1)})
                                this.setState({currenentPage: (e.nativeEvent.contentOffset.x > 0 ? 1 : 0)})
                            }}
                >
